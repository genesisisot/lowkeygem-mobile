...
import {
  Bell, Menu, X, User, Briefcase, Wallet, HelpCircle, Settings,
  LogOut, Home, Heart, MessageSquare, Bookmark, type LucideIcon,
} from 'lucide-react'
...
const navLinks: { icon: LucideIcon; view: string; href: string }[] = [
  { icon: Home, view: 'dashboard', href: '/client/dashboard' },
  { icon: Heart, view: 'discover', href: '/client/discover' },
  { icon: MessageSquare, view: 'matches', href: '/client/matches' },
  { icon: Bookmark, view: 'saved', href: '/client/saved' },
]

const menuLinks: { icon: LucideIcon; label: string; href: string | null }[] = [
  { icon: User, label: 'Profile', href: '/client/my-profile' },
  { icon: Briefcase, label: 'My Jobs', href: '/client/my-jobs' },
  { icon: Wallet, label: 'Wallet', href: '/client/wallet' },
  { icon: HelpCircle, label: 'Help & Support', href: '/client/support' },
  { icon: Settings, label: 'Settings', href: '/client/my-profile' },
]

export function ClientLayout({ activeView, onLogout, children }: ClientLayoutProps) {
  const { user } = useAuth()
  ...
  return (
    <div className="w-full overflow-x-hidden" style={{ fontFamily: 'var(--font-effra)' }}>
      {/* Header */}
      <header
              ...
              <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                      <motion.a
                        key={n.id}
                        href={n.type === 'proposal' ? '/client/my-jobs' : '/client/matches'}
                        onClick={(e: React.MouseEvent) => { e.preventDefault(); markAsRead(n.id); setShowNotifications(false); handleNavigate(n.type === 'proposal' ? '/client/my-jobs' : '/client/matches') }}
                        style={{
                    ...
                    {menuLinks.map((item) =>
                      item.href ? (
                        <a
                          ...
                          style={{
                            textDecoration: 'none',
                            background: activeView === item.href.replace('/client/', '') ? 'rgba(151,7,71,0.08)' : 'none',
                          }}
                        >
      ...
      {/* Main Content */}
      <main className={`flex-1 overflow-hidden ${activeView === 'discover' ? 'sm:flex-1 fixed inset-x-0 top-0 sm:relative z-30' : ''}`}
        style={activeView === 'discover' ? { bottom: '58px' } : {}}
      >
        <div className="h-full relative overflow-y-auto" style={activeView === 'discover' ? {} : {}}>
          {children}
        </div>
      </main>
